...
public class Ungrant extends Command implements IConsoleCommand {

    public Ungrant() {
        super("ungrant", Permission.MANAGE_ROLES);
    }
    ...
    public void run(final @NotNull CommandReader reader, final @NotNull ConsoleSender sender) throws NoPermissionException, WrongUsageException {
        ...
        if (!member.getRoles().contains(role)) {
            sender.replyTl("command.ungrant.nothing", member.getUser().getAsTag(), role.getName(), guild.getName());
            return;
        }
        guild.removeRoleFromMember(member, role).complete();
        sender.replyTl("command.ungrant.done", role.getName(), member.getUser().getAsTag(), guild.getName());
    }
}
